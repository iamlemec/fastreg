...
def test_ols(data):
    ret0 = fr.ols(y=y0, x=x0, data=data)
    ret1 = fr.ols(y=y, x=x, data=data)

    assert ~np.isnan(ret0).any().any()
    assert ~np.isnan(ret1).any().any()

def test_ols_statsmodels(data):
    ...
    ret_sm = ret_sm.rename({'Intercept': 'I'}, axis=0).sort_index()
